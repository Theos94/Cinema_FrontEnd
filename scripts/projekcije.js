const kalendarProjekcijeKontejner = document.querySelector(
  ".kalendar__projekcije--kontejner"
);
      ...
      if (projekcija.datum === datum) {
        ...
        div.innerHTML = `
                <div class="projekcija__info">
                    ...
                    <h4 class="pocetak"><span>Pocetak:</span>${
                      projekcija.pocetak
                    }</h4>
                    <h4><span>Sala:</span> ${projekcija.sala}</h4>
                    <h4><span>Mesta:</span> ${projekcija.slobodno} / ${
          projekcija.mesta
        }</h4>
                </div>
...
const overlayDodajCloseBtn = document.querySelector(
  ".overlay__dodaj--closeBtn"
);
    ...
    if (film.id === parseInt(id)) {
      film.projekcije.forEach(projekcija => {
        if (
          ...
          if (
            novaVrednost > projekcija.mesta ||
            novaVrednost > projekcija.slobodno
          ) {
            formaSell.reset();
            swal({
              title: "Neuspesno.",
            ...
            console.log(
              (projekcija.slobodno = projekcija.slobodno - novaVrednost)
            );
