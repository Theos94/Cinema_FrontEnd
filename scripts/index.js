const repertoarProjekcijeKontejner = document.querySelector(
  ".repertoar__projekcije"
);
...
function uzmiDatume() {
  ...
  repertoarProjekcijeKontejner.setAttribute("data-datum", `${datumi[0]}`);
  popuniKontejner(repertoarProjekcijeKontejner, datumi[0]);
}

uzmiDatume();

function popuniKontejner(kontejner, datum) {
  let projekcijaKontejner = kontejner;
  let projekcije;
  if (localStorage.getItem("projekcije") === null) {
    projekcije = [];
  } else {
...
window.addEventListener("load", event => contentLoaded());

function contentLoaded() {
  // const loader = document.querySelector(".site__loader");
